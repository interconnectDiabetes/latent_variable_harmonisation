...
#set up plot
svg(filename = paste0('baselines_cont_pub_', numLevels,'_level_',validation_size,'_valid.svg'), width = 6, height = 8)
...
#set up plot
svg(filename = paste0('split_cat_for_pub_', numLevels,'_level_',validation_size,'_valid_50bl_cv.svg'), width = 6, height = 4)
par(mar=c(4.1,4.1,2.1,2.1))
...
for (i in 1:numbaselines){
  ...
  for (measurement_error_counter in error_vect) {  
    ...
    estimates_cross = numeric()
    std_errors_cross = numeric()
    
    for (j in 1:crossvalidations){
      ...
      estimates_cross = c(estimates_cross, estimate_corrected)
      std_errors_cross = c(std_errors_cross, delta_std_error_corrected)
    }
    
    #print(estimates_cross)
    
    # estimates_cross contains crossvalidations (e.g. 100) estimates.Take the median and corresponding SE
    
    #gets the median value position in the data frame. crossvalidations must be even or it will break.
    cross_median_ind = which((estimates_cross) == median(estimates_cross))

    estimates_cat = c(estimates_cat, estimates_cross[cross_median_ind])
    std_errors_cat = c(std_errors_cat, std_errors_cross[cross_median_ind])
...
estimates_all = data.frame()
std_error_all = data.frame()
lambda_all = data.frame()
naive_all = data.frame()
...
for (i in 1:numbaselines){
  ...
  for (measurement_error_counter in error_vect) {  
    ...
    naive_cat = c(naive_cat, estimate_naive)
    
  }
  
  estimates_all = rbind(estimates_all, estimates_cat)
  std_error_all = rbind(std_error_all, std_errors_cat)
  lambda_all = rbind(lambda_all, lambda_cat)
  naive_all = rbind(naive_all, naive_cat)
}
...
text(usr[2], usr[4], "Estimate [95% CI]", adj = c(1, 6),cex=1)
text(usr[1], usr[4], "Instrument error size", adj = c( 0, 6 ),cex=1)
title(main = paste0('Rep cat. instr. study size ', study_size, ' levels ', numLevels, ' validation ', validation_size ), line = -0.5, adj = 0.5)
...
# Note that the cross validation through repeating the analysis for random splits is
# not included here because it massively increases the run time. As we are only testing
# the Taylor approximation, the results of this single run can be compared
# to a single CV run of the bootstrapped code
...
#set up plot
svg(filename = paste0('split_boot_cat_for_pub_', numLevels,'_level_',validation_size,'_valid_no_cv.svg'), width = 6, height = 4)
